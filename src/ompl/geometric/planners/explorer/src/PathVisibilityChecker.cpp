...
public:
    ...
    // stateDistances vector computes the overall pathLength which is equal to the last entry of stateDistances
...
bool PathVisibilityChecker::IsPathVisible(std::vector<BundleSpaceGraph::Vertex> &v1,
                                          std::vector<BundleSpaceGraph::Vertex> &v2, BundleSpaceGraph::Graph &graph)
{
...
//    std::make_shared<ompl::control::SimpleDirectedControlSampler>(siC, samples); return sampler;
//}
...
bool PathVisibilityChecker::IsPathVisibleSimple(std::vector<BundleSpaceGraph::Vertex> &v1,
                                                std::vector<BundleSpaceGraph::Vertex> &v2,
                                                BundleSpaceGraph::Graph &graph)
{
    ...
    return IsPathVisibleSimple(s1, s2);
}
bool PathVisibilityChecker::IsPathVisibleSimple(std::vector<ob::State *> &s1, std::vector<ob::State *> &s2)
{
    // Disable logging for Checker
    ompl::msg::LogLevel logLevelInitial = ompl::msg::getLogLevel();
    ompl::msg::setLogLevel(ompl::msg::LOG_NONE);
    ...
    ob::State *state1 = si_->allocState();
    ob::State *state2 = si_->allocState();


    computePathLength(si_, s1, d1, path_1_length);
    computePathLength(si_, s1, d1, path_2_length);

    computePathLength(si_, s1, path_1_distances, path_1_length);
    computePathLength(si_, s2, path_2_distances, path_2_length);

    double dist1 = path_1_length / parts;
    double dist2 = path_2_length / parts;

    for (int i = 1 ; i < parts; i++)
    {
        createStateAt(si_, s1, path_1_length, path_1_distances, dist1 * i, state1);
        createStateAt(si_, s2, path_2_length, path_2_distances, dist2 * i, state2);

        if (!si_->checkMotion(state1, state2))
        {
            return false;
        }
